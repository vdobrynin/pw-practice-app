...
test.describe('Form Layouts page', () => {     // that was before at #33
    ...
    test('radio buttons', async ({ page }) => {                 // #34
        const usingTheGridForm = page.locator('nb-card', { hasText: "Using the Grid" })
        // await usingTheGridForm.getByLabel('Option 1').check({ force: true }) //--> 'getByLabel' to click use {force: true}--> cause it's visual hidden
        await usingTheGridForm.getByRole('radio', { name: "Option 1" })
            .check({ force: true }) //--> 'getByRole' to click use {force: true}--> cause it's visual hidden
        const radioStatus = await usingTheGridForm.getByRole('radio', { name: "Option 1" })
            .isChecked()
        expect(radioStatus).toBeTruthy()                                        // #34
        await expect(usingTheGridForm.getByRole('radio', { name: "Option 1" }))
            .toBeChecked()

        await usingTheGridForm.getByRole('radio', { name: "Option 2" }) // #34          // --> comment at #74 ???
            .check({ force: true })
        ...
        // expect(radioStatus).toBeTruthy()                   // --> comment at #74
        // await expect(usingTheGridForm
...
test('checkboxes', async ({ page }) => {
    ...
    for (const box of await allBoxes.all()) {
        await box.check({ force: true })                        // check all boxes
